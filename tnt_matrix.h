...
class Matrix 
{
  ...
  public:
    ...
    */
    ...
    Matrix<T>(Matrix<T>&& a) : m_(a.m_), // move constructor
        n_(a.n_),
        mn_(a.mn_),
        v_(a.v_),
        row_(a.row_),
    ...
    Matrix<T>& operator=(Matrix<T>&& a) // move assignment
    {
        if (this != &a)
        {
             destroy();
...
template <class T>
inline Matrix<T> matmult_block(const Matrix<T> &A,
    const Matrix<T> &B)
{
    const int blockSize=4;
    const int n = A.num_rows();
    const int m = B.num_cols();
    const int p = B.num_rows();

    T zero = 0;
    Matrix<T> result(n, m, zero);
